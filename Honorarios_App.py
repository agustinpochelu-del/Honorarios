...
# ELIMINAMOS EL CACHÉ POR COMPLETO: Siempre lee el archivo fresco del disco.
def cargar_y_procesar_datos(ruta_archivo):
    ...
    # Conservamos dinámicamente las columnas que existan (para evitar KeyErrors si alguna falta)
    cols_deseadas = ['Fecha_dt', 'Mes_Indice', 'Emisor', 'Negocio', 'Tipo', 'Punto de Venta', 'Número Desde', 'Nro. Doc. Receptor', 'Denominación Receptor', 'Facturacion $', 'Facturacion $ Actualizada']
    cols_presentes = [c for c in cols_deseadas if c in df_final.columns]
    df_historial_visual = df_final[cols_presentes].copy()
...
# --- BLINDAJE DE COLUMNAS PARA EVITAR KEYERRORS ---
if 'Emisor' not in df_motor_interno.columns:
    df_motor_interno['Emisor'] = 'Agustín'
if 'Negocio' not in df_motor_interno.columns:
    df_motor_interno['Negocio'] = 'Estudio'

# Creamos columnas internas en mayúsculas para que los filtros funcionen siempre (aunque en el excel digan "laura", "Laura" o "LAURA")
df_motor_interno['Emisor_Clean'] = df_motor_interno['Emisor'].astype(str).str.strip().str.upper()
df_motor_interno['Negocio_Clean'] = df_motor_interno['Negocio'].astype(str).str.strip().str.upper()
...
if pd.notna(fecha_max):
    ...
    def nivel_alerta_afip(emisor_buscado, cat_actual):
        df_e = df_motor_interno[df_motor_interno['Emisor_Clean'].str.contains(emisor_buscado, na=False)]
        fact_12m_nominal = df_e[(df_e['Fecha_dt'] > fecha_12m) & (df_e['Fecha_dt'] <= fecha_max)]['Facturacion $'].sum()
        fact_3m_nominal = df_e[(df_e['Fecha_dt'] > fecha_3m) & (df_e['Fecha_dt'] <= fecha_max)]['Facturacion $'].sum()
    ...
    nivel_ag = nivel_alerta_afip('AGUST', st.session_state['cat_agustin'])
...
with st.sidebar:
    ...
    MENU_CLIENTES = f"{estado_honorarios} Análisis de Clientes"
    MENU_AFIP = f"{estado_afip} Control Monotributo"
    MENU_FACTURAS = "🧾 Historial de Facturas"
    MENU_INDICES = "📊 Tabla de Índices"
    
    opciones_menu = [MENU_ESTADISTICAS, MENU_CYGNUS, MENU_CLIENTES, MENU_AFIP, MENU_FACTURAS, MENU_INDICES]
    seleccion_pantalla = st.radio("Ir a sección:", opciones_menu, label_visibility="collapsed")
    
    st.divider()
    
    # Como quitamos el caché, este botón es magia pura. Vuelve a leer el disco al instante.
...
if seleccion_pantalla == MENU_ESTADISTICAS:
    ...
    st.caption("Esta vista abarca toda la facturación del Estudio (excluyendo Cygnus Home).")
    
    df_estudio = df_motor_filtrado[df_motor_filtrado['Negocio_Clean'].str.contains('ESTUDIO', na=False)]
    df_estudio_absoluto = df_motor_interno[df_motor_interno['Negocio_Clean'].str.contains('ESTUDIO', na=False)]
    
    total_nominal = df_estudio['Facturacion $'].sum()
    total_actualizado = df_estudio['Facturacion $ Actualizada'].sum()
    mes_maximo_real = df_estudio_absoluto['Mes_Indice'].max()
    
    if pd.notna(mes_maximo_real):
        mes_hace_12_real = mes_maximo_real - pd.DateOffset(months=11)
        fact_ult_mes = df_estudio_absoluto[df_estudio_absoluto['Mes_Indice'] == mes_maximo_real]['Facturacion $ Actualizada'].sum()
        fact_ult_12m = df_estudio_absoluto[df_estudio_absoluto['Mes_Indice'] >= mes_hace_12_real]['Facturacion $ Actualizada'].sum()
        ...
        if not df_estudio.empty:
            ...
            df_evolucion_mensual = df_estudio.groupby('Año-Mes')[['Facturacion $', 'Facturacion $ Actualizada']].sum().reset_index()
            ...
            st.divider()
            df_ranking_clientes = df_estudio.groupby('Denominación Receptor')['Facturacion $ Actualizada'].sum().reset_index().sort_values(by='Facturacion $ Actualizada', ascending=True)
    ...
    st.caption("Métricas filtradas exclusivamente para el negocio Cygnus Home.")
    
    df_cygnus = df_motor_filtrado[df_motor_filtrado['Negocio_Clean'].str.contains('CYGNUS', na=False)]
    df_cygnus_absoluto = df_motor_interno[df_motor_interno['Negocio_Clean'].str.contains('CYGNUS', na=False)]
    
    if df_cygnus_absoluto.empty:
        st.info("Aún no hay facturas registradas en la base de datos para Cygnus Home.")
    else:
        mes_curso = df_cygnus_absoluto['Mes_Indice'].max()
        mes_anterior = mes_curso - pd.DateOffset(months=1)
        f_12m = mes_curso - pd.DateOffset(months=11)
        
        fact_mes_curso = df_cygnus_absoluto[df_cygnus_absoluto['Mes_Indice'] == mes_curso]['Facturacion $ Actualizada'].sum()
        fact_mes_anterior = df_cygnus_absoluto[df_cygnus_absoluto['Mes_Indice'] == mes_anterior]['Facturacion $ Actualizada'].sum()
    ...
    if not clientes_vencidos.empty:
        st.error(f"⚠️ **Atención:** Hay **{len(clientes_vencidos)}** clientes con honorarios atrasados. Modificá la columna 'Nuevo Precio Pactado'.")
    else:
    ...
    if mes_actual in [6, 12]:
        ...
        st.error("🚨 **Período de Recategorización Activo:** Tenés tiempo hasta el día 20 de este mes para recategorizarte en AFIP.")
        
    def renderizar_afip(emisor_buscado, nombre_display, cat_actual, col):
        with col:
            st.write(f"### 👤 {nombre_display}")
            df_emisor = df_motor_interno[df_motor_interno['Emisor_Clean'].str.contains(emisor_buscado, na=False)]
            if df_emisor.empty:
                ...
                st.info(f"📍 Categoría AFIP Sugerida: **{cat_sug}**")

    col_izq, col_der = st.columns(2)
    renderizar_afip('AGUST', 'Agustín', st.session_state['cat_agustin'], col_izq)
    renderizar_afip('LAURA', 'Laura', st.session_state['cat_laura'], col_der)
        
    st.divider()
    st.write("### 🛠️ Corrección de Categoría Actual")
    st.write("Si te recategorizaste o la letra está mal, cambiala acá para recalcular las alertas:")
    ...
    # Filtramos la tabla usando las columnas internas limpias
    if vista_facturas == "🏢 Estudio Contable":
        df_hist_render = df_historial_filtrado[df_motor_filtrado['Negocio_Clean'].str.contains('ESTUDIO', na=False)].copy()
    elif vista_facturas == "🏠 Cygnus Home":
        df_hist_render = df_historial_filtrado[df_motor_filtrado['Negocio_Clean'].str.contains('CYGNUS', na=False)].copy()
    else:
